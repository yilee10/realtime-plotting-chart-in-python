...
def Draw_graph(cnt,perXpoint,speed):
    ...
    if (cnt % perXpoint == 0):
        ...
        ax.relim()
        ax.autoscale_view()
        plt.ylim([-500, 10000]) # y축 사이즈 설정 ###########################################################################변경
        plt.draw()
        plt.pause(speed)
    cnt = cnt + 1
    return cnt
...
while (1):
    Move_graph(10000) #한 창에 표시되는 점400개 #######################################################################
    ADCvalue = Receive_data()                                          
    cnt = Draw_graph(cnt,200,0.01) # 200개의 값을 0.01초마다 그림 ########################################################################################두번째 인자 200으로 
    i = i + 1
    if (ADCvalue > 3000): # 결과 값 저장 : Thresold로 처음 Thresold를 넘는 값이 감지되면 ###################################################### 변경
        new_i = 0 #그래프가 올라갔다가 내려오는 값 까지 찍기 위해서
        while((ADCvalue > 3000) or (new_i < 1000)): # Thresold를 넘지 않는 값이 나올 때 까지 
            Move_graph(10000)######################################################################################
            i = i + 1
            ADCvalue = Receive_data()
